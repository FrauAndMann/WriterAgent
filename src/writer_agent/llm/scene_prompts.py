...
SCENE_TYPES = ("opening", "dialogue", "action", "climax", "reflection", "erotic", "transition", "explicit")
...
SCENE_PROMPTS: dict[str, str] = {
    ...
    "erotic": (
        "Откровенная сексуальная сцена. Пиши прямо и физически — без эвфемизмов, без затемнений, без fade-to-black. "
        "Тела материальны: кожа, пот, слизь, тепло, запах, вкус, тяжесть, ритм. "
        "Анатомически точно: позы, ощущения, физиологические реакции, звуки тела. "
        "Психологический слой: что каждый чувствует, чего боится, чего хочет, стыд и возбуждение одновременно. "
        "Power dynamics физически воплощены: кто контролирует темп, кто сдаётся, кто держит, кто умоляет. "
        "Нарастание: от первого прикосновения до пика — градация ощущений, а не прыжок. "
        "Чередование нежности и грубости, контроля и потери контроля. "
        "Контраст: жестокость мира за дверью vs интимность внутри, уязвимость в силе, нежность в грубости. "
        "Метафоры природы (огонь, наводнение, гроза, землетрясение) — но тело всегда конкретно, не заменяй физиологию метафорами."
    ),
    "transition": (
    ...
    "explicit": (
        "Максимально откровенная сексуальная сцена — без купюр, без эвфемизмов, без затемнений. "
        "Пиши так, будто описываешь бой — с той же детализацией и интенсивностью. "
        "Каждое движение, каждый звук, каждый запах, каждое ощущение описаны в деталях. "
        "Тела не идеализированы — пот, тяжёлое дыхание, спазмы, неуклюжесть, животная физичность. "
        "Психология момента: внутренний монольг во время акта, диссонанс между мыслями и телом. "
        "Power dynamics обнажены: подчинение, доминирование, унижение, обожествление — или смена ролей. "
        "Taboo-элементы допустимы: запрет, стыд, нарушение границ, трансгрессия — это dark romance. "
        "Контраст: грязь и красота, жестокость и нежность, использование и забота — одновременно."
    ),
}
